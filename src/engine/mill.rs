...
impl U1Gauge3D {
    ...
    fn get_tx(&self, x: usize, y: usize, z: usize) -> f64 {
        self.tx[self.idx(x, y, z)]
    }

    fn get_ty(&self, x: usize, y: usize, z: usize) -> f64 {
        self.ty[self.idx(x, y, z)]
    }

    fn get_tz(&self, x: usize, y: usize, z: usize) -> f64 {
        self.tz[self.idx(x, y, z)]
    }

    fn set_tx(&mut self, x: usize, y: usize, z: usize, v: f64) {
        let idx = self.idx(x, y, z);
        self.tx[idx] = v;
    }

    fn set_ty(&mut self, x: usize, y: usize, z: usize, v: f64) {
        let idx = self.idx(x, y, z);
        self.ty[idx] = v;
    }

    fn set_tz(&mut self, x: usize, y: usize, z: usize, v: f64) {
        let idx = self.idx(x, y, z);
        self.tz[idx] = v;
    }

    fn plaquette_angle_xy(&self, x: usize, y: usize, z: usize) -> f64 {
        let xp = self.wrap(x as isize + 1);
        let yp = self.wrap(y as isize + 1);
        self.get_tx(x, y, z) + self.get_ty(xp, y, z) - self.get_tx(x, yp, z) - self.get_ty(x, y, z)
    }

    fn plaquette_angle_xz(&self, x: usize, y: usize, z: usize) -> f64 {
        let xp = self.wrap(x as isize + 1);
        let zp = self.wrap(z as isize + 1);
        self.get_tx(x, y, z) + self.get_tz(xp, y, z) - self.get_tx(x, y, zp) - self.get_tz(x, y, z)
    }

    fn plaquette_angle_yz(&self, x: usize, y: usize, z: usize) -> f64 {
        let yp = self.wrap(y as isize + 1);
        let zp = self.wrap(z as isize + 1);
        self.get_ty(x, y, z) + self.get_tz(x, yp, z) - self.get_ty(x, y, zp) - self.get_tz(x, y, z)
    }

    fn plaquette_cos_xy(&self, x: usize, y: usize, z: usize) -> f64 {
        self.plaquette_angle_xy(x, y, z).cos()
    }

    fn plaquette_cos_xz(&self, x: usize, y: usize, z: usize) -> f64 {
        self.plaquette_angle_xz(x, y, z).cos()
    }

    fn plaquette_cos_yz(&self, x: usize, y: usize, z: usize) -> f64 {
        self.plaquette_angle_yz(x, y, z).cos()
    }
...
fn run_mill_with_rp(
    ...
    let mut field = U1Gauge3D::new(cfg.l);
    ...
    MillRunOutput {
        trace_id: format!("MILL_U1_3D_L{}_b{}_seed{}", cfg.l, cfg.beta, cfg.seed),
...
fn local_cos_sum_for_link_x(cfg: &U1Gauge3D, x: usize, y: usize, z: usize) -> f64 {
...
fn local_cos_sum_for_link_y(cfg: &U1Gauge3D, x: usize, y: usize, z: usize) -> f64 {
...
fn local_cos_sum_for_link_z(cfg: &U1Gauge3D, x: usize, y: usize, z: usize) -> f64 {
    let xm = cfg.wrap(x as isize - 1);
    let ym = cfg.wrap(y as isize - 1);
    cfg.plaquette_cos_xz(x, y, z)
...
fn metropolis_sweep(cfg: &mut U1Gauge3D, beta: f64, step_size: f64, rng: &mut StdRng) {
    let l = cfg.l;
    let n_links = 3 * l * l * l;
    for _ in 0..n_links {
...
fn translation_invariance_row_dev(cfg: &U1Gauge3D) -> f64 {
...
fn reflection_positivity_estimate(cfg: &U1Gauge3D) -> f64 {
    let l = cfg.l;
    if l < 2 {
        return f64::NAN;
    }
...
impl OperatorSmearingAccumulator {
    ...
    fn observe(&mut self, field: &U1Gauge3D) {
...
fn ape_smear_u1(field: &U1Gauge3D, alpha: f64, steps: usize) -> U1Gauge3D {
    if steps == 0 {
        return field.clone();
    }
        ...
        for z in 0..l {
            for y in 0..l {
                for x in 0..l {
                    ...
                    let u = c_exp_i(cur.get_tx(x, y, z));
                    let staple_y_f =
                        c_exp_i(cur.get_ty(x, y, z) + cur.get_tx(x, yp, z) - cur.get_ty(xp, y, z));
                    let staple_y_b = c_exp_i(
                        -cur.get_ty(x, ym, z) + cur.get_tx(x, ym, z) + cur.get_ty(xp, ym, z),
                    );
                    let staple_z_f =
                        c_exp_i(cur.get_tz(x, y, z) + cur.get_tx(x, y, zp) - cur.get_tz(xp, y, z));
                    let staple_z_b = c_exp_i(
                        -cur.get_tz(x, y, zm) + cur.get_tx(x, y, zm) + cur.get_tz(xp, y, zm),
                    );
                    let staples = c_add(c_add(staple_y_f, staple_y_b), c_add(staple_z_f, staple_z_b));
                    let v = c_add(c_scale(u, 1.0 - alpha), c_scale(staples, alpha * 0.25));
                    next.set_tx(x, y, z, c_arg(v));

                    let u = c_exp_i(cur.get_ty(x, y, z));
                    let staple_x_f =
                        c_exp_i(cur.get_tx(x, y, z) + cur.get_ty(xp, y, z) - cur.get_tx(x, yp, z));
                    let staple_x_b = c_exp_i(
                        -cur.get_tx(xm, y, z) + cur.get_ty(xm, y, z) + cur.get_tx(xm, yp, z),
                    );
                    let staple_z_f =
                        c_exp_i(cur.get_tz(x, y, z) + cur.get_ty(x, y, zp) - cur.get_tz(x, yp, z));
                    let staple_z_b = c_exp_i(
                        -cur.get_tz(x, y, zm) + cur.get_ty(x, y, zm) + cur.get_tz(x, yp, zm),
                    );
                    let staples = c_add(c_add(staple_x_f, staple_x_b), c_add(staple_z_f, staple_z_b));
                    let v = c_add(c_scale(u, 1.0 - alpha), c_scale(staples, alpha * 0.25));
                    next.set_ty(x, y, z, c_arg(v));

                    let u = c_exp_i(cur.get_tz(x, y, z));
                    let staple_x_f =
                        c_exp_i(cur.get_tx(x, y, z) + cur.get_tz(xp, y, z) - cur.get_tx(x, y, zp));
                    let staple_x_b = c_exp_i(
                        -cur.get_tx(xm, y, z) + cur.get_tz(xm, y, z) + cur.get_tx(xm, y, zp),
                    );
                    let staple_y_f =
                        c_exp_i(cur.get_ty(x, y, z) + cur.get_tz(x, yp, z) - cur.get_ty(x, y, zp));
                    let staple_y_b = c_exp_i(
                        -cur.get_ty(x, ym, z) + cur.get_tz(x, ym, z) + cur.get_ty(x, ym, zp),
                    );
                    let staples = c_add(c_add(staple_x_f, staple_x_b), c_add(staple_y_f, staple_y_b));
                    let v = c_add(c_scale(u, 1.0 - alpha), c_scale(staples, alpha * 0.25));
                    next.set_tz(x, y, z, c_arg(v));
                }
            }
        }
        cur = next;
...
fn region_plaquette_mean(field: &U1Gauge3D, x0: usize, x1: usize, y0: usize, y1: usize) -> f64 {
    let mut s = 0.0;
    let mut n = 0usize;
    for z in 0..field.l {
        for y in y0..y1 {
...
fn region_neighbor_corr_x(field: &U1Gauge3D, y0: usize, y1: usize) -> f64 {
    let l = field.l;
    let mut s = 0.0;
    let mut n = 0usize;
    for z in 0..l {
...
fn region_neighbor_corr_x_reflect_y(field: &U1Gauge3D, l: usize, y0: usize, y1: usize) -> f64 {
    let ll = field.l;
    let mut s = 0.0;
    let mut n = 0usize;
    for z in 0..ll {
...
fn f_vec(field: &U1Gauge3D, l: usize) -> [f64; 3] {
    let half = l / 2;
...
fn theta_f_vec(field: &U1Gauge3D, l: usize) -> [f64; 3] {
    let half = l / 2;
